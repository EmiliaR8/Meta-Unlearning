...
parser = argparse.ArgumentParser(description="EMBER 2024 Continual Learning Experiment (MADAR-IF + ER/KD/SI)")
...
def update_buffer_madar(loader, model):
    ...
    # Pure MADAR uses UNIFORM budgeting across families
...
def train_cl_er(model, teacher_model, optimizer, loader, iters, active_count,
    ...
    for step in range(iters):
        ...
        if step in probe_steps:
            # .detach() before float(): loss_main is graph-attached, and si_loss is
            # an int 0 on the first CL task before omega is populated.
...
if __name__ == "__main__":
    ...
    for tid, current_fams in enumerate(task_families):
        ...
        if tid == 0:
            ...
            if args.task0_checkpoint and os.path.exists(args.task0_checkpoint):
                # map_location='cpu' keeps the replay buffer on CPU, matching
                # freshly built entries (a mixed-device buffer breaks torch.stack).
            ...
            # Initialise the SI anchors and replay buffer only after task 0
            for n, p in model.named_parameters():
                if p.requires_grad: p_old_task[n.replace('.', '__')] = p.detach().clone()
    ...
    tag = f"cap{args.family_cap}_{args.arch}_seed{SEED}"
    if args.cl_iters != 2000: tag += f"_it{args.cl_iters}"
    if args.si_c != 2.0: tag += f"_si{args.si_c}"
    with open(f'ember24_madar_history_{tag}.json', 'w') as f:
    ...
    plt.title(f'EMBER 2024 MADAR-IF + ER/KD/SI ({args.arch}, seed {SEED})')
    plt.legend(); plt.grid(alpha=0.3)
    plt.savefig(f'ember24_cl_madar_{tag}.png')

    plt.figure(figsize=(10, 6))
    for fam in TARGET_FAMS:
        plt.plot(fam_history[fam]['acc'], marker='o', label=f'family {fam}')
    plt.xlabel('Task'); plt.ylabel('Per-family recall (%)')
    plt.title(f'EMBER 2024 MADAR: tracked family retention (seed {SEED})')
    plt.legend(fontsize=9); plt.grid(alpha=0.3)
    plt.savefig(f'ember24_madar_families_{tag}.png')