"""ARCH STUDY -- MADAR-IF + ER/KD/SI (no unlearning).

Derived from clmd_ember18_mk9_cd.py. Identical training logic; the only changes are that the
...
parser.add_argument('--task0-only', dest='task0_only', action='store_true',
                    help='Train task 0, report its held-out accuracy, and exit before the '
                         'CL phase. Used for the architecture range check: if task 0 '
                         'collapses at a given width, any forgetting measured on top of '
                         'it is meaningless. Writes no JSON.')
...
if __name__ == "__main__":
    ...
    for tid, current_fams in enumerate(task_families):
        ...
        if tid == 0 and args.task0_only:
            print(f"\n[range check] arch={args.arch} {ARCH_DIMS} "
                  f"({count_params(INPUT_DIM, 100, ARCH_DIMS):,} params) "
                  f"-> task-0 accuracy {t0_acc:.2f}%")
            print("[range check] --task0-only set; exiting before the CL phase "
                  "(no JSON written).")
            raise SystemExit(0)
    ...
    history['config'] = {
        'condition': 'madar', 'arch': args.arch, 'arch_dims': list(ARCH_DIMS),
        'n_params': count_params(INPUT_DIM, 100, ARCH_DIMS), 'seed': SEED,
        'input_dim': INPUT_DIM, 'num_classes': 100, 'si_c': SI_C,
    }
    out_json = f'{OUT_DIR}/madar_{ARCH_TAG}_seed{SEED}.json'
    ...
    plt.title(f'MADAR-IF + ER/KD/SI - arch {args.arch} (seed {SEED})')
    plt.legend(); plt.grid(alpha=0.3)
    plt.savefig(f'{OUT_DIR}/madar_{ARCH_TAG}_seed{SEED}.png')