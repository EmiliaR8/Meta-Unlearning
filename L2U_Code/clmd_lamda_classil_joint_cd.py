"""
Usage:
    python clmd_lamda_classil_joint_cd.py --seed 42
    python clmd_lamda_classil_joint_cd.py --seed 42 --split-mode temporal \
...
parser = argparse.ArgumentParser(description="LAMDA Class-IL (Joint Retraining Oracle)")
...
parser.add_argument('--family-cap', type=int, default=0,
                    help='cap TRAIN samples per family (0 = off; protocol default is OFF '
                         'for LAMDA -- see LAMDA_PROTOCOL.md). Kept for parity with the '
                         'EMBER 2024 scripts.')
...
def get_loader(X, y, families, drop_last=False):
    # drop_last=True on training loaders so a final batch of size 1 can never
    # crash BatchNorm in train mode (identical to the EMBER scripts).
...
def eval_acc_macro(model, X, y, families, active_count):
    """Mean per-family recall. HEADLINE METRIC (EMBER 2024 protocol).
    LAMDA families are 128:1 imbalanced, so micro accuracy is dominated by the
    largest few. Vectorised with bincount rather than a per-family Python loop."""
    model.eval()
    mask = torch.isin(y, torch.tensor(families))
    if not mask.any():
        return 0.0
...
if __name__ == "__main__":
    ...
    # NOTE: no model is built here. The joint oracle constructs a FRESH model
    # inside the task loop. Instantiating one early would consume torch RNG draws
    # (weight init) and shift task 0's initialisation relative to the EMBER joint
    # script, so n_params is reported from the first in-loop model instead.
    n_params = None
    ...
    for tid, current_fams in enumerate(task_families):
        ...
        # drop_last=True: the joint pool changes size every task, giving 11
        # chances to hit N % BATCH_SIZE == 1 and crash BatchNorm in train mode.
        loader = get_loader(X_train, y_train, seen_fams, drop_last=True)
        if loader is None:
        ...
        # Throw away the old model and start fresh -- this is what makes it an
        # oracle: no parameter carry-over, so nothing can be forgotten.
        model = EmberNN(INPUT_DIM, NUM_CLASSES).to(DEVICE)
        if n_params is None:
            n_params = sum(p.numel() for p in model.parameters())
            print(f"  [model] EmberNN({INPUT_DIM} -> 1024 -> 512 -> 256 -> 128 -> "
                  f"{NUM_CLASSES})  params={n_params:,}  (rebuilt each task)")
    ...
    plt.xlabel('Task')
    plt.ylabel('Accuracy (%)')
    plt.title(f'LAMDA Class-IL Joint Oracle (seed {SEED}, {args.split_mode} split)')
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.title(f'LAMDA Class-IL Joint Oracle: tracked family recall (seed {SEED})')
