...
RUN_TAG = f"opt{SPLIT_OPTION.upper()}_a{UNLEARN_ALPHA}_r{UNLEARN_RATIO}_seed{SEED}"
...
if __name__ == "__main__":
    ...
    with open(f'ember_mu_history_{RUN_TAG}.json', 'w') as f:
    ...
    plt.title(f'MADAR-IF + ER/KD/SI + Unlearning ({RUN_TAG})')
    plt.legend(); plt.grid(alpha=0.3)
    plt.savefig(f'ember_mu_cl_{RUN_TAG}.png')