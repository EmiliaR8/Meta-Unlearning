...
class Experiment(ABC):
    ...
    def state_dict(self) -> dict:
        """Everything the driver needs to continue at the next task.

        The shared pieces live here; a method with its own persistent state --
        a GAN, a Fisher diagonal, a second buffer -- extends this through
        `extra_state`. Anything a method rebuilds per task (optimisers created
        inside train_task, the frozen `before` copy the unlearning phase makes)
        is deliberately absent: saving it would imply it carries meaning across
        a boundary, and it does not.
        """
        state = {
            ...
            "si": ({"W": self.si.W, "omega": self.si.omega, "p_old": self.si.p_old}
                   if self.si is not None else None),
    ...
    def load_state_dict(self, state: dict) -> None:
        ...
        if state.get("buffer") is not None and self.buffer is not None:
            self.buffer.load_state_dict(state["buffer"])
        if state.get("si") is not None and self.si is not None:
            self.si.W = state["si"]["W"]
            self.si.omega = state["si"]["omega"]
            self.si.p_old = state["si"]["p_old"]
        self.load_extra_state(state.get("extra") or {})

    def extra_state(self) -> dict:
        """Method-specific state. Empty for methods that keep none."""
        return {}
    ...
    # -- driver -----------------------------------------------------------
    def run(self, record: RunRecord, checkpoint=None) -> RunRecord:
        """Train and evaluate every task.

        `checkpoint`, when given, is a (store, config_hash) pair plus whatever
        was loaded; the loop writes one after each completed task and starts at
        the task after the last one recorded. Resuming skips ONLY tasks whose
        record entry is already present, so the JSON a resumed run writes has
        exactly the entries a straight-through run would have.
        """
        record.model_info = self.model_info()
        n_tasks = self.ctx.n_tasks
        start = 0
        if checkpoint is not None and checkpoint.get("payload"):
            payload = checkpoint["payload"]
            self.load_state_dict(payload["experiment"])
            record.per_task = list(payload["record"]["per_task"])
            record.notes = list(payload["record"]["notes"])
            ckpt_mod.set_rng_state(payload.get("rng") or {})
            record.adopt_elapsed(payload["record"].get("elapsed", 0.0))
            start = int(payload["last_task"]) + 1
            record.note(f"resumed from checkpoint after task {payload['last_task']}")
            if not self.cfg.get("quiet"):
                print(f"  resuming at task {start} "
                      f"({len(record.per_task)} task(s) already recorded)",
                      flush=True)
        for tid in range(start, n_tasks):
            ...
            if checkpoint is not None:
                ckpt_mod.save(checkpoint["store"], record.run_id,
                              checkpoint["config_hash"], tid=tid,
                              experiment=self, record=record)
        return record
