...
class MalCLExperiment(Experiment):
    ...
    def __init__(self, ctx):
        if len(getattr(ctx, "input_shape", ())) == 3:
            raise SystemExit(
                "malcl is not available on an image corpus. Its generator and "
                "discriminator are 1-D convolutional networks over a feature "
                "VECTOR; on images they would synthesise a flat 12,288-length "
                "signal with no spatial structure, which is not MalCL adapted to "
                "images but a different method sharing its name. Generative "
                "replay for images needs an image generator (DCGAN/diffusion), "
    ...
    def _mean_logits(self, sums, counts):
        """Per-class mean of the trunk responses seen during the task.

        Takes a running SUM and COUNT rather than the full list of vectors it
        used to be handed. The old form appended every sample's 5,760-wide
        response, for every one of the 50 epochs, and for the replay rows too --
        then averaged the lot and discarded it. On EMBER 2018 that is hundreds of
        gigabytes of Python-owned tensors per task for a quantity that is O(class
        x width), and it is what made long MalCL runs die of memory exhaustion
        partway through. Summing in float64 makes the result if anything more
        accurate than mean-over-stack; the two agree to ~1e-8 in float32.
        ...
        for i in range(len(counts)):
            ...
            rows.append((sums[i] / n).float())
        return (torch.stack(rows) if rows else None), skipped

    # -- resumable state --------------------------------------------------
    def extra_state(self) -> dict:
        """MalCL carries more across a boundary than any other method here.

        The GAN and the classifier each have their own optimiser, and unlike
        every other method's, these are built ONCE in __init__ and reused for
        every task -- so their Adam/SGD moment estimates are genuine cross-task
        state, not per-task scratch. `past_G` and `past_C` are the frozen
        snapshots generative replay draws from, and `logits_real` is the
        per-class mean the selector matches against; a resumed run missing any
        of the three would silently produce replay from an untrained generator.
        """
        return {
            "G": self.G.state_dict(), "D": self.D.state_dict(),
            "G_opt": self.G_opt.state_dict(), "D_opt": self.D_opt.state_dict(),
            "C_opt": self.C_opt.state_dict(),
            "past_G": self.past_G.state_dict() if self.past_G is not None else None,
            "past_C": self.past_C.state_dict() if self.past_C is not None else None,
            "past_C_output_dim": (self.past_C.output_dim
                                  if self.past_C is not None else None),
    ...
    def load_extra_state(self, state: dict) -> None:
        if not state:
            return
        # The classifier head GROWS with each task (expand_output_layer), so the
        # saved width has to be restored before any state_dict is loaded into it.
        # The base class has already loaded `model`, which means this widening
        # must have happened first -- see load_state_dict below.
        ...
        self.C_opt.load_state_dict(state["C_opt"])
        if state.get("past_G") is not None:
            self.past_G = copy.deepcopy(self.G)
            self.past_G.load_state_dict(state["past_G"])
        if state.get("past_C") is not None:
            self.past_C = copy.deepcopy(self.model)
            if state.get("past_C_output_dim") is not None:
                self.past_C.output_dim = state["past_C_output_dim"]
            self.past_C.load_state_dict(state["past_C"])
        self.logits_real = state.get("logits_real")

    def load_state_dict(self, state: dict) -> None:
        """Widen the classifier head before the base class loads weights into it.

        MalCL replaces `fc1`/`fc1_bn1` with wider layers at each task boundary,
        so a fresh model built for task 0 cannot accept a checkpoint taken after
        task 5: the shapes disagree. The saved width is applied first, then the
        normal load proceeds.
        """
        target = (state.get("extra") or {}).get("model_output_dim")
        if target is not None and int(target) != int(self.model.output_dim):
            init = self.schedule.task0
            step = self.schedule.step
            done = (int(target) - init) // step if step else 0
            self.model.expand_output_layer(init, step, done)
            self.model.to(self.device)
            # C_opt is deliberately NOT rebuilt here. PRESERVED DEFECT (2) IS
            # EXACTLY THAT IT IS NEVER REBUILT: the optimiser keeps the param
            # groups it was given in __init__, which after an expansion no longer
            # contain the live head. Rebuilding it on resume would repair the
            # defect and make a resumed run a different method from an
            # uninterrupted one. Its saved state loads into the same stale
            # structure a straight-through run would have had at this point.
            pass
    ...
    def train_task(self, tid: int, loader) -> dict:
        ...
        # Running accumulator for the per-class mean response; see _mean_logits.
        # Width is not known until the first forward, so the sum buffer is
        # allocated lazily rather than hardcoding the trunk's output size.
        logit_sums, logit_counts = None, torch.zeros(n_class, dtype=torch.long)
        losses, steps = [], 0
        ...
        for _ in range(self.hp["malcl_epochs"]):
            for xb, yb in train_loader:
                ...
                with torch.no_grad():
                    ...
                    cls = torch.max(y, dim=1)[1].cpu()
                    if logit_sums is None:
                        logit_sums = torch.zeros(n_class, vec.shape[1],
                                                 dtype=torch.float64)
                    logit_sums.index_add_(0, cls, vec.double())
                    logit_counts.index_add_(0, cls, torch.ones_like(cls))

        self.past_G = copy.deepcopy(self.G)
        self.past_C = copy.deepcopy(self.model)
        if logit_sums is None:                      # a task with no batches
            logit_sums = torch.zeros(n_class, 1, dtype=torch.float64)
        self.logits_real, skipped = self._mean_logits(logit_sums, logit_counts)
        if skipped:
