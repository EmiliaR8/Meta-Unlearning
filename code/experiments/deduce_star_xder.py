...
class DeduceStarXDerExperiment(StarXDerExperiment):
    ...
    def extra_state(self) -> dict:
        # Everything DEDUCE carries ACROSS a task boundary. `_step`, `_conflict`
        # and `anchor` are reset at the start of each task by design, so they
        # are not here -- saving them would resurrect mid-task bookkeeping into a
        # run that is about to clear it anyway. `prev_task_params` and the GUM
        # counters are not reset, and losing them would silently disable the
        # proximal term and hand every neuron a fresh maturity clock.
        state = super().extra_state()
        state.update({
            "fisher": {"F": self.fisher.F, "P": self.fisher.P,
                       "n_batches": self.fisher.n_batches,
                       "raw_mean": self.fisher.raw_mean},
            "prev_task_params": self.prev_task_params,
            "gum": {"C": self.gum.C, "age": self.gum.age,
                    "credit": self.gum.credit,
                    "gen": self.gum.gen.get_state()},
        })
        return state
    ...
    def load_extra_state(self, state: dict) -> None:
        ...
        if f:
            self.fisher.F = f["F"]
            self.fisher.P = f["P"]
            self.fisher.n_batches = f["n_batches"]
            self.fisher.raw_mean = f["raw_mean"]
        if state.get("prev_task_params"):
            self.prev_task_params = state["prev_task_params"]
        g = state.get("gum")
        if g:
            self.gum.C = g["C"]
            self.gum.age = g["age"]
            self.gum.credit = g["credit"]
            self.gum.gen.set_state(
                torch.as_tensor(g["gen"], dtype=torch.uint8, device="cpu"))
